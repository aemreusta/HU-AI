...
void apply_gaussian_smoothing(const char* input_image, int kernel_size, double sigma) {
    ...
    std::ostringstream sigma_stream;
    sigma_stream << std::fixed << std::setprecision(1) << sigma;
    
    std::string output_filename = "gaussian_filtered_" + remove_extension(input_image) + "_" + std::to_string(kernel_size) + "_" + sigma_stream.str() + ".png";
    img.save_to_file(output_filename.c_str());
}
...
void apply_unsharp_mask(const char* input_image, int kernel_size, double amount) {
    ...
    amount_stream << std::fixed << std::setprecision(1) << amount;

    std::string output_filename = "unsharp_filtered_" + remove_extension(input_image) + "_" + std::to_string(kernel_size) + "_" + amount_stream.str() + ".png";
    img.save_to_file(output_filename.c_str());
}
