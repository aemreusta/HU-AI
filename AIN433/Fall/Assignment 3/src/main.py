...
DATASET_NAME = "/Users/emre/GitHub/HU-AI/AIN433/Assignment 3/datasets/raw/"
...
def main():
    for subset_name, subset_images_names in subsets.items():
        ...
        H = None
        panorama = cv2.imread(
            DATASET_NAME + "/" + subset_name + "/" + subset_images_names[0]
        )
        ...
        for i in range(0, len(subset_images_names) - 1):
            ...
            next_image = cv2.imread(
                DATASET_NAME + "/" + subset_name + "/" + subset_images_names[i + 1]
            )
            ...
            cur_image = cv2.imread(
                DATASET_NAME + "/" + subset_name + "/" + subset_images_names[i]
            )
        ...
        # plt.savefig(DATASET_NAME + "/" + subset_name + "_output/Panorama.png")
        plt.show()


def stitch_images(
    cur_image, cur_image_gray, next_image, next_image_gray, feature_points_plot
):
    # Feature extraction
    cur_feature_pts, cur_descs, feature_points_plot = feature_extraction(
        cur_image, cur_image_gray, feature_points_plot
    )
    next_feature_pts, next_descs, feature_points_plot = feature_extraction(
        next_image, next_image_gray, feature_points_plot
    )
